...
| Sector capital rotation (`sector_rotation`) | State Street's own free NAV-history files (`ingestion/src/sources/ssga.ts`), 11 Select Sector SPDRs + SPY + GLD — NAV price return and an estimated creation/redemption flow (share-count change × NAV) at 30/90/180/365 days | Added 2026-09-15 after a long search for a *real* flow signal (not a price proxy) for sector-level capital rotation. Every commercial fund-flow source checked (ICI, ETF.com, EPFR/Lipper/BofA, a third-party ETF.com scraper) was either paid or bot-blocked; the issuer's own regulatory-disclosure NAV-history file turned out to be free, unauthenticated, and genuinely dynamic day-to-day (share-count changes only reflect real creation/redemption, unlike AUM or price) | `flow_estimate_usd` is ETF-**vehicle**-level, not a complete picture of money entering/leaving the underlying sector — investors can get the same exposure through other ETFs (e.g. QQQ/VGT/SMH instead of XLK). `nav_return_pct` is price-only, not total return (dividends excluded — a fast-follow, not v1; Massive's `/stocks/v1/dividends` endpoint was separately verified working). CFTC COT and Fed Z.1 (also investigated) measure positioning and macro-level transactions respectively, not sector-level flow — kept as separate, deferred items, not folded into this one |
